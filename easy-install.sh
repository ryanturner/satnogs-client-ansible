...
echo -n "Calibration Band (GSM850 maybe?): "
read CALIBRATION_BAND
...
cd ..
echo "[satnogs]
localhost SATNOGS_API_URL=$SATNOGS_API_URL SATNOGS_API_TOKEN=$SATNOGS_API_TOKEN SATNOGS_STATION_ID=$SATNOGS_STATION_ID SATNOGS_STATION_LAT=$SATNOGS_STATION_LAT SATNOGS_STATION_LON=$SATNOGS_STATION_LON SATNOGS_STATION_ELEV=$SATNOGS_STATION_ELEV calibration_band=$CALIBRATION_BAND $ADDITIONAL_ANSIBLE_HOSTVARS
" > hosts
echo "- hosts: satnogs
  serial: 1
  become: yes
